...
class Model_products extends CI_Model
{
	...
	function get_product_addon($prod)
	{
		$sql = "SELECT * FROM addon WHERE product_id = ".$prod." ORDER BY id ASC";
			$query = $this->db->query($sql, array(1));
			return $query->result_array();
	}
	...
	public function create_addon($data)
	{
		if($data)
		{
			$insert_addon=$this->db->insert('addon',$data);
			#echo $this->db->last_query();
			return ($insert_addon == true) ? $insert_addon : false;
		}
	}
	...
	public function remove_addon($id)
	{
		if($id) {
			$this->db->where('product_id', $id);
			$delete = $this->db->delete('addon');
