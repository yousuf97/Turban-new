...
class Model_orders extends CI_Model
{
    ...
    function getonlineOrdersData()
    {
			$sql = "SELECT * FROM checkout ";
			$query = $this->db->query($sql);
			return $query->result_array();		
    }
	function getonlineOrdersDataForPrint($id)
    {
	...
	function order_update($id,$table,$data)
	{
		$this->db->where('id', $id);
	   $update = $this->db->update($table, $data);
       return ($update== true)?true:false;
	}
	function get_confirmd_notpaid_order($ordertable,$stat)
	{
		$sql = "SELECT * FROM ".$ordertable." WHERE ".$stat." = 0";
		$query = $this->db->query($sql, array(1));
		return $query->result_array();
	}
	...
	function order_not_confirmed($table,$status)
	{
